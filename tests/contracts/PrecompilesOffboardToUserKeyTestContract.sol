...
contract PrecompilesOffboardToUserKeyTestContract {
    ...
    function getCTs() public view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        return (ct8, ct16, ct32, ct64, ct128, ct256High, ct256Low);
    }
    ...
    function offboardToUserTest(uint8 a, address addr) public returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        gtUint128 a128_s = MpcCore.setPublic128(a);
        gtUint256 a256_s = MpcCore.setPublic256(a);
        ...
        ctUint128 cipher128 = MpcCore.offBoardToUser(a128_s, addr); 
        ctUint256 memory cipher256 = MpcCore.offBoardToUser(a256_s, addr);

        ct8 = ctUint8.unwrap(cipher8);
        ct16 = ctUint16.unwrap(cipher16);
        ct32 = ctUint32.unwrap(cipher32);
        ct64 = ctUint64.unwrap(cipher64);
        ct128 = ctUint128.unwrap(cipher128);
        ct256High = ctUint128.unwrap(cipher256.ciphertextHigh);
        ct256Low = ctUint128.unwrap(cipher256.ciphertextLow);

        return (ct8, ct16, ct32, ct64, ct128, ct256High, ct256Low);
    }

    function offboardOnboardTest(uint8 a8, uint16 a16, uint32 a32, uint32 a64, uint128 a128, uint256 a256) public returns (uint8) {
        ...
        gtUint128 a128_s = MpcCore.setPublic128(a128);
        gtUint256 a256_s = MpcCore.setPublic256(a256);
        ...
        ctUint128 cipher128 = MpcCore.offBoard(a128_s);
        uint128 result128 = MpcCore.decrypt(MpcCore.onBoard(cipher128));

        ctUint256 memory cipher256 = MpcCore.offBoard(a256_s);
        uint256 result256 = MpcCore.decrypt(MpcCore.onBoard(cipher256));

        require(result == result16 && result == result32 && result == result64 && result == result128 && result == result256, 
