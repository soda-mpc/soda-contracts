...
class SodaWeb3Helper:
    ...
    def deploy_contract(self, 
        ...
        construct_txn = self._build_transaction(func_to_call, gas_limit, gas_price, chain_id, constructor_args)
        receipt = self._sign_and_send_transaction(construct_txn)
    ...
    def call_contract_transaction(self, 
        ...
        transaction = self._build_transaction(func_to_call, gas_limit, gas_price, chain_id, func_args)
    ...
    def _build_transaction(self, func, gas_limit, gas_price, chain_id, tx_args=[]):
        print(f'Deploying contract with args: {tx_args}')
...
def compile_contract(file_path):
    ...
    compiled_sol = compile_standard({
        "language": "Solidity",
        "sources": {file_path: {"content": solidity_code}},
        "settings": {
            "outputSelection": {
                "*": {
