...
export const LOCAL_PROVIDER_URL = 'http://localhost:7000'
export const REMOTE_HTTP_PROVIDER_URL = 'https://node.sodalabs.net' 
...
export class SodaWeb3Helper {
    ...
    async deployContract(contractId, constructorArgs = [], gasLimit = DEFAULT_GAS_LIMIT, gasPrice = DEFAULT_GAS_PRICE, chainId = DEFAULT_CHAIN_ID) {
        ...
        const constructTxn = {
            ...
            nonce: nonce,
            gas: gasLimit,
            gasPrice: this.web3.utils.toWei(gasPrice.toString(), 'wei')
        };
    ...
    updateContractAddress(contractId, contractAddress) {
        if (!(contractId in this.contracts)) {
            throw new Error(`Contract with id ${contractId} does not exist. Use the 'setupContract' method to set it up.`);
        }

        this.contracts[contractId] = new this.web3.eth.Contract(
            this.contracts[contractId].options.jsonInterface,
            contractAddress,
            { data: this.contracts[contractId].options.data }
        );
        return true;
    }
    ...
    async estimateGas(contractId, funcName, funcArgs = []) {
        if(process.env.DISABLE_GAS_ESTIMATION === 'true'){
            return 0;
        }
        if (!(contractId in this.contracts)) {
            console.log(`Contract with id ${contractId} does not exist. Use the 'setupContract' method to set it up.`);
    ...
    async callContractTransaction(contractId, funcName, funcArgs = [], gasLimit = DEFAULT_GAS_LIMIT, gasPrice = DEFAULT_GAS_PRICE, chainId = DEFAULT_CHAIN_ID) {
        ...
        const contract = this.contracts[contractId];
        const funcToCall = contract.methods[funcName](...funcArgs);
        return this.callContractFunctionTransaction(funcToCall, gasLimit, gasPrice, chainId);
    }
