...
export const REMOTE_HTTP_PROVIDER_URL = 'http://node.sodalabs.net:7000' 
...
export class SodaWeb3Helper {
    ...
    async setupContract(contractPath, contractName, contractId, overwrite = false) {
        ...
        try {
            ...
            console.log(`Contract with id ${contractId} set up successfully.`);
            return true;
        } catch (error) {
    ...
    compileContract(filePath, fileName, mpcInstPath = "lib/solidity/MPCInst.sol", mpcCorePath = "lib/solidity/MpcCore.sol") {
    ...
    async deployContract(contractId, constructorArgs = [], gasLimit = DEFAULT_GAS_LIMIT, gasPrice = DEFAULT_GAS_PRICE, chainId = DEFAULT_CHAIN_ID) {
        ...
        console.log(`Deploying contract: ${contractId}, Gas limit: ${gasLimit}, Gas price: ${gasPrice}, Chain id: ${chainId}`);
    ...
    async callContractFunctionTransaction(func, gasLimit = DEFAULT_GAS_LIMIT, gasPrice = DEFAULT_GAS_PRICE, chainId = DEFAULT_CHAIN_ID) {
        // const transaction = await this.buildTransaction(gasLimit, gasPrice, chainId);
        const nonce = await this.web3.eth.getTransactionCount(this.account.address);
        const transaction = {
            from: this.account.address,
            chainId: chainId,
    ...
    async sendTransaction(funcToCall, transaction) {
        try {
