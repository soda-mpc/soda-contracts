...
export class SodaWeb3Helper {
    ...
    async callContractFunctionTransaction(func, gasLimit = DEFAULT_GAS_LIMIT, gasPrice = DEFAULT_GAS_PRICE, chainId = DEFAULT_CHAIN_ID) {
        const nonce = await this.web3.eth.getTransactionCount(this.account.address) + BigInt(async_tx);
        const transaction = {
            from: this.account.address,
            chainId: chainId,
            nonce: nonce,
    ...
    async callContractFunctionTransactionAsync(func, contract, gasLimit = DEFAULT_GAS_LIMIT, gasPrice = DEFAULT_GAS_PRICE, chainId = DEFAULT_CHAIN_ID) {
        const nonce = await this.web3.eth.getTransactionCount(this.account.address) + BigInt(async_tx);
        const transaction = {
            from: this.account.address,
            chainId: chainId,
