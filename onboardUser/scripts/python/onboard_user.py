...
def main(provider_url: str):
    ...
    if default_contract_address:
        ...
        # Deploy the contract
        receipt = soda_helper.deploy_contract(contract_id, constructor_args=[], gas_limit=950_000)
        if receipt is None:
    ...
    receipt = soda_helper.call_contract_transaction(contract_id, "getUserKey", func_args=[public_key, signature], gas_limit=650_000)
    if receipt is None:
        raise Exception("Failed to call the transaction function")

    encryptedKey0, encryptedKey1 = get_user_key_shares(soda_helper.get_account(), contract, receipt)
